...
public class Pathfinding
{
    ...
    private GridMap<GameObject> grid;
    private List<Pipe> openList;
    private List<Pipe> closedList;

    public Pathfinding(GridMap<GameObject> puzzle)
    {
        grid = puzzle;
    }
    ...
    public List<Pipe> FindPath(int startX, int startY, int endX, int endY)
    {
        ...
        Pipe startNode = grid.GetObject(startX, startY).GetComponent<Pipe>();
        Pipe endNode = grid.GetObject(endX, endY).GetComponent<Pipe>();
        ...
        for (int x = 0; x < grid.Width; x++)
        {
            ...
            {
                Pipe pathNode = grid.GetObject(x, y).GetComponent<Pipe>();
    ...
    {
        return grid.GetObject(x, y).GetComponent<Pipe>();
    }
